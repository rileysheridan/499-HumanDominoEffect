...
func _init() :
	super(GdUnit3Version.parse("v1.1.0"))
	pass
...
func execute() -> bool:
	...
	var migrated_template := old_template\
		super.replace("${class_name}", GdUnitTestSuiteTemplate.TAG_TEST_SUITE_CLASS)\
		super.replace("${source_path}", GdUnitTestSuiteTemplate.TAG_SOURCE_RESOURCE_PATH)
