...
@onready var style :StyleBoxFlat = bar.get("theme_override_styles/fg")
...
func _ready():
	...
	bar.custom_minimum_size.y = font_size + 4 * plugin.get_editor_interface().get_editor_scale()
	plugin.free()
...
func progress_update(value :int, failed :int, max_value :int = -1) -> void:
	...
	if failed > 0:
		var style:StyleBoxFlat  = bar.get("theme_override_styles/fg")
