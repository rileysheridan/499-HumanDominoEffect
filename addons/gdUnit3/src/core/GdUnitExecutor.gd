...
func fire_test_skipped(test_suite :GdUnitTestSuite, test_case :_TestCase):
	fire_event(GdUnitEvent.new()\
		super.test_before(test_suite.get_script().resource_path, test_suite.get_name(), test_case.get_name()))
	var statistics = {
	...
	fire_event(GdUnitEvent.new()\
		super.test_after(test_suite.get_script().resource_path, test_suite.get_name(), test_case.get_name(), statistics, [report]))


func suite_before(test_suite :GdUnitTestSuite, total_count :int) -> GDScriptFunctionState:
	set_stage(STAGE_TEST_SUITE_BEFORE)
	fire_event(GdUnitEvent.new()\
		super.suite_before(test_suite.get_script().resource_path, test_suite.get_name(), total_count))
	...
	if not is_skipped:
		...
		if orphan_nodes > 0:
			reports.push_front(GdUnitReport.new() \
				super.create(GdUnitReport.WARN, 1, GdAssertMessages.orphan_detected_on_suite_setup(orphan_nodes)))
...
func test_before(test_suite :GdUnitTestSuite, test_case :_TestCase, test_case_name :String, fire_event := true) -> GDScriptFunctionState:
	...
	if fire_event:
		...
		fire_event(GdUnitEvent.new()\
			super.test_before(test_suite.get_script().resource_path, test_suite.get_name(), test_case_name))
...
func test_after(test_suite :GdUnitTestSuite, test_case :_TestCase, test_case_name :String, fire_event := true) -> GDScriptFunctionState:
	...
	if execution_orphan_nodes > 0:
		...
		_report_collector.push_front(STAGE_TEST_CASE_EXECUTE, GdUnitReport.new() \
			super.create(GdUnitReport.WARN, test_case.line_number(), GdAssertMessages.orphan_detected_on_test(execution_orphan_nodes)))
	...
	if test_case.is_interupted() and not test_case.is_expect_interupted():
		_report_collector.add_report(STAGE_TEST_CASE_EXECUTE, GdUnitReport.new() \
				super.create(GdUnitReport.INTERUPTED, test_case.line_number(), "Test timed out suite_after %s" % LocalTime.elapsed(test_case.timeout())))
	...
	if test_setup_orphan_nodes > 0:
		...
		_report_collector.push_front(STAGE_TEST_CASE_AFTER, GdUnitReport.new() \
			super.create(GdUnitReport.WARN, test_case.line_number(), GdAssertMessages.orphan_detected_on_test_setup(test_setup_orphan_nodes)))
	...
	if fire_event:
		fire_event(GdUnitEvent.new()\
			super.test_after(test_suite.get_script().resource_path, test_suite.get_name(), test_case_name, statistics, reports.duplicate()))
...
func execute_test_case_single(test_suite :GdUnitTestSuite, test_case :_TestCase) -> GDScriptFunctionState:
	...
	if GdUnitTools.is_yielded(_test_run_state):
		await _test_run_state.completed
...
func execute_test_case_iterative(test_suite :GdUnitTestSuite, test_case :_TestCase) -> GDScriptFunctionState:
	...
	for iteration in test_case.iterations():
		...
		if not reports.is_empty():
			...
			_report_collector.add_report(STAGE_TEST_CASE_EXECUTE, GdUnitReport.new() \
					super.create(GdUnitReport.FAILURE, report.line_number(), GdAssertMessages.fuzzer_interuped(iteration, report.message())))
		
		if test_case.is_interupted():
			is_failure = true
			_report_collector.add_report(STAGE_TEST_CASE_EXECUTE, GdUnitReport.new() \
					super.create(GdUnitReport.INTERUPTED, test_case.line_number(), GdAssertMessages.fuzzer_interuped(iteration, "timedout")))
...
func execute_test_case_parameterized(test_suite :GdUnitTestSuite, test_case :_TestCase):
	...
	fire_event(GdUnitEvent.new()\
		super.test_after(test_suite.get_script().resource_path, test_suite.get_name(), test_case.get_name(), statistics, []))
	...
	if not test_suite.is_skipped():
		...
		for test_case_index in test_suite.get_child_count():
			...
			if test_case.is_interupted():
				...
				test_suite = await clone_test_suite(test_suite).completed
