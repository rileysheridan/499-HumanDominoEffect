...
func register_on_client_connected(target :Object, eventFunc :String) -> void:
	var err = connect("client_connected", Callable(target, eventFunc).bind(), CONNECT_REFERENCE_COUNTED)
	if err != OK:
...
func register_on_client_disconnected(target :Object, eventFunc :String) -> void:
	var err = connect("client_disconnected", Callable(target, eventFunc).bind(), CONNECT_REFERENCE_COUNTED)
	if err != OK:
...
func register_on_message(target:Object, eventFunc:String) -> void:
	var err = connect("send_message", Callable(target, eventFunc).bind(), CONNECT_REFERENCE_COUNTED)
	if err != OK:
...
func register_on_test_suite_added(target :Object, eventFunc :String) -> void:
	var err = connect("test_suite_added", Callable(target, eventFunc).bind(), CONNECT_REFERENCE_COUNTED)
	if err != OK:
...
func register_on_gdunit_events(target:Object, eventFunc:String) -> void:
	if is_connected("test_suite_event", Callable(target, eventFunc)):
		return
	var err = connect("test_suite_event", Callable(target, eventFunc))
	if err != OK:
		push_error("Can't connect signal <test_suite_added> to %s:%s.\n%s" % [target, eventFunc, GdUnitTools.error_as_string(err)])

func unregister_on_gdunit_events(target:Object, eventFunc:String) -> void:
	if is_connected("test_suite_event", Callable(target, eventFunc)):
		disconnect("test_suite_event", Callable(target, eventFunc))
